...
# 🔹 Make sure the folder containing main_distance_detector.py is on sys.path
ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
if ROOT not in sys.path:
    sys.path.insert(0, ROOT)

from main_distance_detector import DistanceDetector  # uses YOLO internally :contentReference[oaicite:1]{index=1}

# -------------------- CONFIG --------------------
CLASSES = ["1m", "2m", "3m", "4m", "5m"]
...
def pick_zip_file():
    ...
    path = filedialog.askopenfilename(
        title="Select ZIP with distance images",
...
def plot_confusion_matrix(cm, classes):
    ...
    plt.title("Confusion Matrix (distance: 1m / 2m / 3m / 4m / 5m)")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with zipfile.ZipFile(zip_path, "r") as z:
            for name in z.namelist():
                ...
                out = detector.detect(img, conf=CONF_THRES, iou=IOU_THRES)
                if not out:
                    skipped_pred += 1
                    continue
